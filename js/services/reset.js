(function(){
  ...
  function createEmptyState(current={}){const user=current.usuario||{},next={schemaVersion:9,__centsVersion:1,usuario:{nome:user.nome||'',onboardingIgnorado:Boolean(user.onboardingIgnorado),...(user.preferencias?{preferencias:JSON.parse(JSON.stringify(user.preferencias))}:{})},configuracoesFinanceiras:{reservaMeses:6},planejamentos:{},fechamentos:{},_backups:[]};arrays.forEach(key=>next[key]=[]);return next;}
