(function(){
  ...
  function buildHtml(key,closing){
    ...
    <h2>Orçamento por categoria</h2><table><thead><tr><th>Categoria</th><th class="num">Orçado</th><th class="num">Realizado</th><th class="num">Disponível</th></tr></thead><tbody>${rows(budgets,item=>`<tr><td>${escape(item.categoriaNome)}</td><td class="num">${money(item.orcado)}</td><td class="num">${money(item.realizado)}</td><td class="num">${money(item.disponivel)}</td></tr>`,4,'Nenhum orçamento registrado no snapshot.')}</tbody></table>
