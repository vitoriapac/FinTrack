...
function renderAppFooter(){
  ...
  return `<footer class="app-footer"><div class="footer-head"><div><strong>FinTrack</strong><span>Salvo localmente · ${esc(FinTrackStorage.adapter)}</span></div><nav class="footer-actions" aria-label="Backup e relatórios"><button class="footer-action footer-action-primary" id="footer-backup">Backup JSON</button><label class="footer-action footer-action-primary" for="footer-import">Importar backup</label><input id="footer-import" type="file" accept="application/json,.json" hidden><button class="footer-action" id="footer-report" ${latest?'':'disabled'}>Relatório mensal</button><button class="footer-action" id="footer-recovery">Arquivo de recuperação</button></nav></div><details class="backup-panel"><summary>Backup e recuperação — ferramentas administrativas</summary><div class="backup-panel-body">${renderBackupTab()}</div></details><div class="footer-meta"><span>Dados financeiros não saem deste dispositivo.</span><span><kbd>Ctrl</kbd> + <kbd>K</kbd> Busca · salvamento automático ativo</span></div></footer>`;
}
