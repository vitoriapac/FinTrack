function attachViewHandlers(){
  ...
  main.querySelectorAll('[data-insight-target]').forEach(button=>button.onclick=()=>navigateToInsight({view:button.dataset.insightTarget,month:button.dataset.insightMonth,categoryId:button.dataset.insightCategory,date:button.dataset.insightDate}));
  ...
  const compareScenario=document.getElementById('btn-compare-scenario');if(compareScenario)compareScenario.onclick=()=>{const target=document.getElementById('scenario-result');try{const result=ScenarioService.compare(state,mesAtualKey(),{kind:document.getElementById('scenario-kind').value,amount:toCents(document.getElementById('scenario-amount').value),installments:Number(document.getElementById('scenario-installments').value),debtId:document.getElementById('scenario-debt').value}),chart=FinTrackCharts.multiLine({title:'Saldo projetado: atual × cenário',question:'Qual seria a diferença no caixa?',summary:`Menor saldo atual: ${formatMoney(result.baselineMinimum)} · no cenário: ${formatMoney(result.simulatedMinimum)}.`,data:result.rows.map(item=>({label:item.month,baseline:item.baseline,simulated:item.simulated})),keys:[{value:'baseline',label:'Atual'},{value:'simulated',label:'Cenário'}],format:formatMoney});target.innerHTML=`${chart}<div class="table-wrap"><table><thead><tr><th>Mês</th><th class="num">Atual</th><th class="num">Cenário</th><th class="num">Diferença</th></tr></thead><tbody>${result.rows.map(item=>`<tr><td>${esc(item.month)}</td><td class="num">${formatMoney(item.baseline)}</td><td class="num">${formatMoney(item.simulated)}</td><td class="num">${formatMoney(item.difference)}</td></tr>`).join('')}</tbody></table></div>${result.debt?`<p>Quitação simplificada: ${result.debt.mesesEstimados} mês(es), ${formatMoney(result.debt.totalEstimado)} estimados.</p>`:''}<p>${esc(result.notice)}</p><small>${esc(result.disclaimer)}</small>`;FinTrackCharts.bindTooltips(target);}catch(error){target.textContent=error.message;}};
