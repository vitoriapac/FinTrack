function attachViewHandlers(){
  ...
  main.querySelectorAll('[data-instruction-target]').forEach(button=>button.onclick=()=>{const target=button.dataset.instructionTarget;if(target==='backup')document.getElementById('footer-backup')?.click();else setView(target);});
  ...
  const scenarioKind=document.getElementById('scenario-kind'),scenarioAmountLabel=document.getElementById('scenario-amount-label'),scenarioGuidance=document.getElementById('scenario-guidance');
  const scenarioCopy={
    'reduce-expense':['Economia mensal (R$)','A economia é hipotética: o orçamento cadastrado permanece igual.'],
    reserve:['Aporte mensal (R$)','O aporte reduz o caixa disponível e representa dinheiro direcionado à reserva, não uma despesa.'],
    debt:['Pagamento extra mensal (R$)','O pagamento extra é estimado à parte; as parcelas futuras da dívida não são recalculadas nesta projeção.'],
    installment:['Valor total da compra (R$)','As parcelas são distribuídas igualmente a partir deste mês; juros e datas reais não são simulados.']
  };
  const syncScenarioChoice=()=>{if(!scenarioKind)return;const kind=scenarioKind.value;main.querySelectorAll('[data-scenario-kind]').forEach(button=>button.setAttribute('aria-pressed',String(button.dataset.scenarioKind===kind)));const installmentField=document.getElementById('scenario-installments-field'),debtField=document.getElementById('scenario-debt-field');if(installmentField)installmentField.hidden=kind!=='installment';if(debtField)debtField.hidden=kind!=='debt';if(scenarioAmountLabel)scenarioAmountLabel.textContent=scenarioCopy[kind]?.[0]||'Valor (R$)';if(scenarioGuidance)scenarioGuidance.textContent=scenarioCopy[kind]?.[1]||'';};
  if(scenarioKind){scenarioKind.onchange=syncScenarioChoice;syncScenarioChoice();main.querySelectorAll('[data-scenario-kind]').forEach(button=>button.onclick=()=>{scenarioKind.value=button.dataset.scenarioKind;syncScenarioChoice();});}
  const compareScenario=document.getElementById('btn-compare-scenario');if(compareScenario)compareScenario.onclick=()=>{const target=document.getElementById('scenario-result');try{const result=ScenarioService.compare(state,mesAtualKey(),{kind:scenarioKind.value,amount:toCents(document.getElementById('scenario-amount').value),installments:Number(document.getElementById('scenario-installments').value),debtId:document.getElementById('scenario-debt').value}),difference=result.simulatedMinimum-result.baselineMinimum,impact=difference>0?`O menor saldo melhora em ${formatMoney(difference)}.`:difference<0?`O menor saldo diminui em ${formatMoney(Math.abs(difference))}.`:'O menor saldo permanece igual.',chart=FinTrackCharts.multiLine({title:'Saldo projetado: atual × cenário',question:'Qual seria a diferença no caixa?',summary:`Menor saldo: atual ${formatMoney(result.baselineMinimum)} · cenário ${formatMoney(result.simulatedMinimum)}. ${impact}`,data:result.rows.map(item=>({label:item.month,baseline:item.baseline,simulated:item.simulated})),keys:[{value:'baseline',label:'Plano atual'},{value:'simulated',label:'Cenário'}],format:formatMoney});target.innerHTML=`<div class="scenario-reading"><h3>Como interpretar</h3><p>${esc(impact)} A diferença compara o menor saldo projetado, não uma garantia de resultado. Valores maiores significam mais caixa disponível.</p><p><strong>Menor saldo no plano atual:</strong> ${formatMoney(result.baselineMinimum)}<br><strong>Menor saldo com a hipótese:</strong> ${formatMoney(result.simulatedMinimum)}</p></div>${chart}<h3>Comparação mês a mês</h3><div class="table-wrap"><table><thead><tr><th>Mês</th><th class="num">Plano atual</th><th class="num">Cenário</th><th class="num">Variação do saldo</th></tr></thead><tbody>${result.rows.map(item=>`<tr><td>${esc(item.month)}</td><td class="num">${formatMoney(item.baseline)}</td><td class="num">${formatMoney(item.simulated)}</td><td class="num">${item.difference>0?'+':''}${formatMoney(item.difference)} <span class="scenario-direction">${item.difference>0?'mais caixa':item.difference<0?'menos caixa':'sem diferença'}</span></td></tr>`).join('')}</tbody></table></div>${result.debt?`<div class="scenario-reading"><strong>Estimativa simplificada de quitação:</strong> ${result.debt.mesesEstimados} mês(es), ${formatMoney(result.debt.totalEstimado)} no total.</div>`:''}<p>${esc(result.notice)}</p><small>${esc(result.disclaimer)}</small>`;FinTrackCharts.bindTooltips(target);}catch(error){target.textContent=error.message;}};
  ...
  const reserveTarget=document.getElementById('reserve-target');if(reserveTarget)reserveTarget.onchange=async()=>{FinTrackState.transaction(current=>({...current,configuracoesFinanceiras:{...(current.configuracoesFinanceiras||{}),reservaMeses:Number(reserveTarget.value)}}));await saveData();render();};
  ...
  const footerReport=document.getElementById('footer-report');if(footerReport)footerReport.onclick=()=>{const entries=Object.entries(state.fechamentos||{}).filter(([,item])=>item.status==='fechado'&&item.snapshot).sort(([a],[b])=>b.localeCompare(a));if(entries[0])FinTrackPdfReport.open(entries[0][0],entries[0][1],entries[1]?.[1]);};
  const footerClear=document.getElementById('footer-clear-data');if(footerClear)footerClear.onclick=()=>{if(state.modoDemo){infoModal('Limpeza indisponível','Restaure seus dados pessoais antes de usar esta ação. A demonstração nunca apaga os dados preservados.');return;}const openFinal=()=>FinTrackModal.open('Confirmar limpeza definitiva',`<p>Digite <strong>LIMPAR</strong> para confirmar.</p><div class="field"><label for="clear-confirmation">Confirmação</label><input id="clear-confirmation" autocomplete="off"></div><div class="modal-actions"><button class="btn btn-ghost" id="clear-final-cancel">Cancelar</button><button class="btn danger" id="clear-final" disabled>Limpar definitivamente</button></div>`,()=>{const input=document.getElementById('clear-confirmation'),button=document.getElementById('clear-final');document.getElementById('clear-final-cancel').onclick=FinTrackModal.close;input.oninput=()=>button.disabled=input.value!=='LIMPAR';button.onclick=async()=>{const previous=JSON.parse(JSON.stringify(state)),backups=previous._backups||[],next=normalizeData(DataResetService.createEmptyState(state));FinTrackState.replaceState(next);try{if(!await saveData())throw new Error('Falha ao salvar o estado vazio');await FinTrackStorage.set(BACKUPS_KEY,[]);FinTrackModal.close();render();}catch(error){FinTrackState.replaceState(previous);await saveData();await FinTrackStorage.set(BACKUPS_KEY,backups).catch(()=>{});infoModal('Limpeza não concluída','Não foi possível persistir a alteração; seus dados anteriores foram restaurados.');}};});FinTrackModal.open('Limpar dados do FinTrack',`<p>Esta ação removerá contas, categorias, lançamentos, planejamentos, metas, cartões, dívidas, investimentos e histórico salvos neste navegador.</p><p>Recomendamos criar um backup antes.</p><div class="modal-actions"><button class="btn btn-ghost" id="clear-cancel">Cancelar</button><button class="btn btn-ghost" id="clear-export">Exportar backup antes</button><button class="btn danger-ghost" id="clear-continue">Continuar</button></div>`,()=>{document.getElementById('clear-cancel').onclick=FinTrackModal.close;document.getElementById('clear-export').onclick=exportarJSON;document.getElementById('clear-continue').onclick=()=>{FinTrackModal.close();openFinal();};});};
  ...
  main.querySelectorAll('[data-ignore-recurrence]').forEach(button=>button.onclick=async()=>{const decision={id:uid('recurrence-decision'),fingerprint:button.dataset.ignoreRecurrence,status:'ignorada',sourceIds:(button.dataset.sourceIds||'').split(',').filter(Boolean),serieId:null,decidedAt:new Date().toISOString()};FinTrackState.transaction(current=>({...current,recurrenceDecisions:[...(current.recurrenceDecisions||[]).filter(item=>item.fingerprint!==decision.fingerprint),decision]}));await saveData();render();});
  main.querySelectorAll('[data-confirm-recurrence]').forEach(button=>button.onclick=()=>{const candidate=AnalysisService.recurringExpenses(state,{today:todayLocal()}).evidence.detected.find(item=>item.fingerprint===button.dataset.confirmRecurrence);if(candidate)openEntryForm(null,{prefill:{tipo:'Despesa',descricao:candidate.descricao,valor:candidate.mensal,contaId:candidate.contaId,categoriaId:candidate.categoriaId,status:'Pendente'},recurrenceCandidate:candidate});});
