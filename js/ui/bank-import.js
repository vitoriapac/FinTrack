(function(){
  ...
  const identity=transaction=>transaction.sourceId?`${transaction.source.accountId}|${transaction.sourceId}`:[transaction.source.accountId,transaction.date,transaction.amountCents,transaction.description.toLowerCase()].join('|');
  ...
  async function chooseCsv(file){
    ...
    session={format:'csv',fileName:file.name,parsed,mapping:FinTrackBankCsv.suggestMapping(parsed.headers),accountId:state.contas[0]?.id||'',page:0,decisions:[]};
    showMapping();
  }
  async function chooseOfx(file){
    ...
    if(parsed.errors.length){FinTrackModal.info('OFX não suportado',parsed.errors[0]);return;}
    session={format:'ofx',fileName:file.name,parsed,statementIndex:0,accountId:state.contas[0]?.id||'',page:0,decisions:[]};
    showOfxSelection();
  }
  ...
  function startPreview(preview,errorId){
    ...
    session.decisions=preview.rows.map(row=>{
      ...
      return {rowNumber:row.rowNumber,transaction,candidates,repeated,suggestion,transferHint,action:candidates.length||repeated||transferHint.reason?'review':'import',categoryId:suggestion?.categoryId||'',matchId:'',destinationAccountId:destinations.length===1?destinations[0]:'',counterpartId:'',rememberRule:false};
    });
  ...
  function showMapping(){
    ...
    const body=`<p>Associe as colunas de <strong>${escape(session.fileName)}</strong>. Use valor com sinal ou as duas colunas débito e crédito.</p><div class="bank-map-grid"><label>Conta<select id="bank-account">${accountOptions(session.accountId)}</select></label>${fields.map(([key,label])=>`<label>${label}<select data-bank-map="${key}">${options(key)}</select></label>`).join('')}</div><p id="bank-map-error" class="form-error" role="alert"></p><div class="modal-actions"><button class="btn btn-ghost" id="bank-cancel">Cancelar</button><button class="btn btn-primary" id="bank-preview">Ver prévia</button></div>`;
  ...
  function showOfxSelection(){
    const body=`<p>Selecione o extrato bancário e a conta correspondente no FinTrack. Apenas transações em BRL serão aceitas.</p><div class="bank-map-grid"><label>Extrato no arquivo<select id="bank-ofx-statement">${session.parsed.statements.map(statement=>`<option value="${statement.index}" ${statement.index===session.statementIndex?'selected':''}>${escape(statement.bankId||'Banco')} · conta final ${escape(statement.accountSuffix||'não informado')} · ${statement.records.length} movimento(s) · ${escape(statement.currency||'moeda ausente')}</option>`).join('')}</select></label><label>Conta no FinTrack<select id="bank-account">${accountOptions(session.accountId)}</select></label></div><p id="bank-ofx-error" class="form-error" role="alert"></p><div class="modal-actions"><button class="btn btn-ghost" id="bank-cancel">Cancelar</button><button class="btn btn-primary" id="bank-preview">Ver prévia</button></div>`;
    FinTrackModal.open('Selecionar extrato OFX',body,()=>{
      document.getElementById('bank-cancel').onclick=FinTrackModal.close;
      document.getElementById('bank-preview').onclick=()=>{
        session.statementIndex=Number(document.getElementById('bank-ofx-statement').value);session.accountId=document.getElementById('bank-account').value;
        const statement=session.parsed.statements[session.statementIndex];session.records=statement.records;
        startPreview(FinTrackBankOfx.preview(statement,session.accountId),'bank-ofx-error');
      };
    });
  }
  ...
  function showReview(){
    ...
    FinTrackModal.open(`Revisar extrato ${session.format.toUpperCase()}`,body,()=>{
      ...
      document.getElementById('bank-back').onclick=session.format==='csv'?showMapping:showOfxSelection;
  ...
  function applyEdit(index,element){
    ...
    session.decisions.forEach((decision,rowIndex)=>{
      ...
      const suggestion=FinTrackImportAssistant.suggestCategory(state,transaction),transferHint=FinTrackImportAssistant.transferHints(state,transaction);
      session.decisions[rowIndex]={...decision,transaction,candidates:FinTrackImportBatches.candidates(state,transaction,candidateIndex),repeated,suggestion,transferHint,action:rowIndex===index||repeated&&!decision.repeated?'review':decision.action,categoryId:rowIndex===index&&previous?.type!==transaction.type?suggestion?.categoryId||'':decision.categoryId||suggestion?.categoryId||'',matchId:rowIndex===index?'':decision.matchId,counterpartId:rowIndex===index?'':decision.counterpartId};
    });
    showReview();
  }
  ...
  async function commit(){
    ...
    try{
      const result=FinTrackImportBatches.commit(state,decisions,{format:session.format,fileName:session.fileName,accountId:session.accountId},uid);
  ...
  FinTrackScreenEvents.register(main=>{
    const csvButton=main.querySelector('#btn-importar-extrato'),csvInput=main.querySelector('#bank-csv-file');
    if(csvButton&&csvInput){csvButton.onclick=()=>csvInput.click();csvInput.onchange=async()=>{try{await chooseCsv(csvInput.files[0]);}catch(error){FinTrackModal.info('Falha na leitura',error.message);}csvInput.value='';};}
    const ofxButton=main.querySelector('#btn-importar-ofx'),ofxInput=main.querySelector('#bank-ofx-file');
    if(ofxButton&&ofxInput){ofxButton.onclick=()=>ofxInput.click();ofxInput.onchange=async()=>{try{await chooseOfx(ofxInput.files[0]);}catch(error){FinTrackModal.info('Falha na leitura',error.message);}ofxInput.value='';};}
