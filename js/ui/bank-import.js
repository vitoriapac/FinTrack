(function(){
  ...
  function startPreview(preview,errorId){
    ...
    const seen=new Set(),index=FinTrackImportBatches.candidateIndex(state),assistantIndex=FinTrackImportAssistant.prepare(state);
    session.decisions=preview.rows.map(row=>{
      ...
      const suggestion=FinTrackImportAssistant.suggestCategory(state,transaction,assistantIndex),transferHint=FinTrackImportAssistant.transferHints(state,transaction,assistantIndex);
  ...
  function showReview(){
    ...
    const pending=session.decisions.filter(decision=>decision.action==='review'||decision.action==='invalid').length;
    const filter=session.filter||'all';
    const indices=session.preview.rows.map((_,index)=>index).filter(index=>filter==='exceptions'?['review','invalid'].includes(session.decisions[index].action):filter==='ready'?session.decisions[index].action==='import'&&!session.decisions[index].candidates.length&&!session.decisions[index].repeated&&!session.decisions[index].transferHint.reason&&!session.decisions[index].ambiguousOrigin:true);
    const totalPages=Math.max(1,Math.ceil(indices.length/40));session.page=Math.min(session.page,totalPages-1);
    const start=session.page*40,visible=indices.slice(start,start+40);
    const body=`<p>${session.preview.totalRows} linha(s): ${valid.length} válida(s), ${session.preview.rows.length-valid.length} com erro, ${suspect} sinalizada(s), ${pending} pendente(s). Correspondências são sugestões; nenhuma é vinculada automaticamente.</p><div class="bank-inbox" role="group" aria-label="Filtrar análise"><button class="btn btn-ghost ${filter==='all'?'active':''}" data-bank-filter="all">Todas (${session.preview.rows.length})</button><button class="btn btn-ghost ${filter==='exceptions'?'active':''}" data-bank-filter="exceptions">Caixa de exceções (${pending})</button><button class="btn btn-ghost ${filter==='ready'?'active':''}" data-bank-filter="ready">Prontas (${valid.length-suspect})</button></div><div class="bank-bulk"><label>Categoria para todas as despesas<select id="bank-bulk-debit">${categoryOptions('debit')}</select></label><label>Categoria para todas as receitas<select id="bank-bulk-credit">${categoryOptions('credit')}</select></label><button class="btn btn-ghost" id="bank-apply-categories">Aplicar categorias</button></div><div class="bank-bulk-actions"><button class="btn btn-ghost" id="bank-select-page">Selecionar página</button><button class="btn btn-ghost" id="bank-clear-selection">Limpar seleção</button><button class="btn btn-ghost" id="bank-bulk-import">Importar selecionadas seguras</button><button class="btn btn-ghost" id="bank-bulk-ignore">Ignorar selecionadas</button><span>${valid.filter(decision=>decision.selected).length} selecionada(s)</span></div><div class="bank-review-list">${visible.length?visible.map(index=>rowMarkup(session.preview.rows[index],index)).join(''):'<p>Nenhuma linha neste filtro.</p>'}</div><div class="bank-pages"><button class="btn btn-ghost" id="bank-prev" ${session.page===0?'disabled':''}>Anterior</button><span>Página ${session.page+1} de ${totalPages}</span><button class="btn btn-ghost" id="bank-next" ${session.page+1>=totalPages?'disabled':''}>Próxima</button></div><p id="bank-review-error" class="form-error" role="alert"></p><div class="modal-actions"><button class="btn btn-ghost" id="bank-back">Voltar</button><button class="btn btn-primary" id="bank-commit">Salvar decisões</button></div>`;
    showStep(`Revisar extrato ${session.format.toUpperCase()}`,body,()=>{
      document.querySelectorAll('[data-bank-filter]').forEach(button=>button.onclick=()=>{session.filter=button.dataset.bankFilter;session.page=0;showReview();});
      document.querySelectorAll('[data-bank-row]').forEach(element=>{
        const decision=session.decisions[Number(element.dataset.bankRow)];
        const selected=element.querySelector('[data-bank-selected]');if(selected)selected.onchange=event=>{decision.selected=event.target.checked;document.querySelector('.bank-bulk-actions span').textContent=`${valid.filter(item=>item.selected).length} selecionada(s)`;};
        const action=element.querySelector('[data-bank-action]');if(action)action.onchange=event=>{decision.action=event.target.value;element.querySelector('[data-bank-transfer-fields]').hidden=decision.action!=='transfer';element.querySelector('[data-bank-category]').disabled=decision.action!=='import';const remember=element.querySelector('[data-bank-remember]');remember.disabled=decision.action!=='import'||FinTrackImportAssistant.isGeneric(decision.transaction.description);if(remember.disabled){remember.checked=false;decision.rememberRule=false;}};
      ...
      document.getElementById('bank-select-page').onclick=()=>{visible.forEach(index=>{if(session.decisions[index].transaction)session.decisions[index].selected=true;});showReview();};
      document.getElementById('bank-clear-selection').onclick=()=>{session.decisions.forEach(decision=>{decision.selected=false;});showReview();};
      document.getElementById('bank-bulk-import').onclick=()=>{const selected=valid.filter(decision=>decision.selected);if(!selected.length){message('bank-review-error','Selecione ao menos uma linha.');return;}if(selected.some(decision=>decision.candidates.length||decision.repeated||decision.transferHint.reason||decision.ambiguousOrigin)){message('bank-review-error','A seleção contém exceções. Revise-as individualmente antes de importar.');return;}FinTrackModal.confirmAction(`Importar ${selected.length} linha(s) segura(s) como novas? Confirme as categorias antes de salvar o lote.`,()=>{selected.forEach(decision=>{decision.action='import';decision.selected=false;});showReview();},'Aplicar decisão');};
      document.getElementById('bank-bulk-ignore').onclick=()=>{const selected=valid.filter(decision=>decision.selected);if(!selected.length){message('bank-review-error','Selecione ao menos uma linha.');return;}FinTrackModal.confirmAction(`Ignorar ${selected.length} linha(s)? Elas não serão criadas neste lote.`,()=>{selected.forEach(decision=>{decision.action='ignore';decision.selected=false;decision.rememberRule=false;});showReview();},'Ignorar linhas');};
  ...
  function applyEdit(index,element){
    ...
    const candidateIndex=FinTrackImportBatches.candidateIndex(state),assistantIndex=FinTrackImportAssistant.prepare(state),seen=new Set();
  ...
  FinTrackScreenEvents.register(main=>{
    ...
    const ruleButton=main.querySelector('#btn-bank-rules');if(ruleButton)ruleButton.onclick=()=>setView('configuracoes:importacoes');
