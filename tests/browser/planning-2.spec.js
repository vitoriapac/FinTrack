...
test('agenda, patrimônio e planejamento futuro usam os contratos do 2.0',async({page})=>{
  ...
  await page.getByRole('button',{name:'Agenda',exact:true}).click();await page.locator('#agenda-mes').fill('2026-12');await page.getByRole('tab',{name:'Lista'}).click();await expect(page.getByText('Seguro futuro',{exact:true})).toBeVisible();
  await page.evaluate(()=>setView('patrimonio'));await expect(page.getByText('Tesouro Direto')).toBeVisible();await expect(page.getByText('Somente snapshots v4')).toBeVisible();
  await page.evaluate(()=>setView('planejamento'));await page.locator('#planejamento-mes').selectOption('2026-10');await expect(page.getByText('Out de 2026')).toBeVisible();
  await page.evaluate(()=>setView('projecao'));await expect(page.getByRole('heading',{name:'Fluxo projetado'})).toBeVisible();await expect(page.getByText('Não altera dados reais')).toBeVisible();
